...
function ProgramDetail({ program }) {
  const roomIcons = ["compass", "search", "layers-3", "briefcase-business", "users", "sparkles"];
  const roomActions = ["Define the company view", "Turn signals into evidence", "Connect discovery to delivery", "Build a repeatable market motion", "Make ownership visible", "Prepare the next conversation"];
  return <SiteShell>
    ...
    <section className="public-scene public-content split-story" data-camera-label="Who it is for"><SectionIntro eyebrow="Who it is for" title="A focused environment for the work directly ahead." /><div className="statement-list sequential-list">{program.fit.map((item, index) => <article className="sequence-row" key={item}><span>0{index + 1}</span><p>{item}</p></article>)}</div></section>
    <section className="public-scene public-content" data-camera-label="Learning architecture"><SectionIntro eyebrow="Learning architecture" title="Six connected rooms. One company-building system." body="The modules are designed to connect rather than compete for founder attention." /><div className="module-grid learning-room-grid">{program.modules.map(([title, body], index) => <article className={`motion-card learning-room-card learning-room-card--${index + 1}`} tabIndex="0" key={title}><span>{String(index + 1).padStart(2,"0")}</span><h3>{title}</h3><p>{body}</p><footer><SiteIcon name={roomIcons[index]} /><strong>{roomActions[index]}</strong><SiteIcon name="arrow-up-right" /></footer></article>)}</div></section>
    ...
    <section className="public-scene public-content outcome-room" data-camera-label="What changes"><SectionIntro eyebrow="What changes" title="Leave with decisions, systems, and a clearer next move." /><ul className="sequential-list">{program.outcomes.map((outcome) => <li className="sequence-row" key={outcome}><span>→</span>{outcome}</li>)}</ul></section>
...
function NetworkPage() {
  const layers=[["01","Founder peers","A trusted room for comparing the difficult work, not performing certainty.","users","Compare the decision with founders at a similar stage"],["02","Operators","Functional experience across product, commercial systems, technology, hiring, and organisational design.","briefcase-business","Bring specialist experience into the question"],["03","Capital relationships","Context-aware introductions when the company, evidence, and timing make the conversation useful.","network","Prepare a relevant, evidence-led introduction"],["04","Market access","Customers, partners, and ecosystem relationships that can help test or extend a credible route to market.","globe-2","Test buyer access and market transfer"],["05","Learning infrastructure","Programs, resources, events, and founder practices that turn access into repeatable capability.","layers-3","Turn a useful moment into a working practice"],["06","Local roots, global reach","Polish and European operating context connected to global founder ambition and market opportunity.","map-pin","Connect local context to the next market"]];
  return <SiteShell><SceneHero number="N" eyebrow="Network ecosystem" title="Access matters when context arrives with it." body="Odra connects founders with peers, operators, capital relationships, market pathways, and learning infrastructure around the decision in front of the company." image="/assets/odra-global-network-map.png" actions={[{label:"Explore founder support",href:"/founder-support"},{label:"Meet at an event",href:"/events"}]} /><section className="public-scene public-content" data-camera-label="Network layers"><SectionIntro eyebrow="Six connected layers" title="A network designed around useful movement." /><div className="network-layer-grid interactive-network-grid">{layers.map(([n,t,b,icon,action],index)=><article className={`motion-card network-layer-card network-layer-card--${index + 1}`} tabIndex="0" key={n}><header><span>{n}</span><SiteIcon name={icon}/></header><h3>{t}</h3><p>{b}</p><footer><strong>{action}</strong><SiteIcon name="arrow-up-right"/></footer></article>)}</div></section><section className="public-scene network-map-room" data-camera-label="How access moves"><img src="/assets/odra-global-network-map.png" alt="Abstract map representing Odra's connected founder network"/><div className="motion-heading"><p className="kicker">How access moves</p><h2>Question → context → relationship → next action.</h2><p>The aim is not the largest possible introduction list. It is the smallest useful set of conversations around a clear founder need.</p></div></section><PageCTA title="Tell us which conversation could change the next decision." body="Founder, operator, ecosystem, and event enquiries can begin through one clear contact route." primary={{label:"Contact Odra",href:"/contact"}} secondary={{label:"Explore programs",href:"/programs"}} /></SiteShell>;
}
